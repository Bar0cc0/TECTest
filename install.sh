...
# Create the systemd log directory
...
run_directly() { 
	...
	echo -e "${YELLOW}--------------------------------------------${NC}"
	sudo -u datauser python3 $CURRENT_DIR/src/main.py
	echo -e "${YELLOW}--------------------------------------------${NC}"
...
launch_test_mode() {
	...
	echo -e "${YELLOW}-----------------------${NC}"
	cd "$func_current_dir"
	sudo -u datauser python3 "$func_current_dir/src/main.py" --test
	echo -e "${YELLOW}-----------------------${NC}"
